...
public static void greet(Socket client) {
    question = 0;
    String message = "\n\rWelcome to MultiPlay Client v"+version+"!\n\r"
                    +"                   by Hyena 2016\n\r"
                    +"\n\r";
    message+=questions[question]+(answers[question].length() > 0 ? (" (default: "+answers[question]+")") : "")+"\n\r";
    try {
        client.getOutputStream().write(message.getBytes(Charset.forName("UTF-8")));
    } catch (IOException e) {
        bug(e.toString());
    }
}
...
public static void interpret(Socket client, byte[] command) {
    if (question < questions.length) {
        if (command.length > 0) answers[question] = new String(command);
        else send(client, answers[question]+"\n\r");
        question++;

        if (question < questions.length) {
            send(client, "\n\r"+questions[question]);
            if (answers[question].length() > 0) send(client, " (default: "+answers[question]+")");
            send(client, "\n\r");
        }
    ...
    if (command.length > 0 && command[0] == '$') {
        ...
        if (multiplay != null) send_bytes(multiplay, command);
        else                   send(client, "You are not connected to the MultiPlay server.\n\r");
    }
    else {
        if (server != null) {
            send_bytes(server, command); send_byte(server, '\n');
            if (multiplay != null && broadcasting && (new String(command)).trim().length() > 0) {
                send(multiplay, "broadcast ");
                send_bytes(multiplay, command);
                send_byte(multiplay, '\n');
            }
        }
        else send(client, "You are not connected to the MUD server.\n\r");
    }
}

public static void send(Socket to, String message) {
    try {
        to.getOutputStream().write(message.getBytes(Charset.forName("UTF-8")));
    } catch (IOException e) {
        bug(e.toString());
    }
}
...
public static boolean step_client() {
    ...
    try {               
        ...
        if (next_byte == -1) log("Connection #"+client_nr+" lost.");
    } catch (SocketTimeoutException e) {
        return false;
    } catch (IOException e) {
        log("An error occurred while reading from connection #"+client_nr+".");
        bug(e.toString());
    }
...
public static boolean step_multiplay() {
    if (client == null || initializing) return false;

    if (multiplay == null) {
        send(client, "Connecting to "+answers[MPS_HOST]+":"+answers[MPS_PORT]+".\n\r");
        if ( (multiplay = connect_to(answers[MPS_HOST], answers[MPS_PORT])) == null) {
            send(client, "Failed to connect to "+answers[MPS_HOST]+":"+answers[MPS_PORT]+" (multiplay).\n\r");
            longsleep=true;                
            try {
                client.getOutputStream().flush();
            } catch (IOException e) {
        ...
        else send(multiplay, "prompt\nclaim "+answers[MPC_NAME]+"\nteam "+answers[MPC_PASS]+"\n");
        return false;
    }
    ...
    try {
        ...
        if (next_byte == -1) log("MultiPlay server disconnected us.");
    } catch (SocketTimeoutException e) {
        return false;
    } catch (IOException e) {
    ...
    if (next_byte != -1) {
        if (next_byte >= 0 && next_byte <= 255) {
            ...
            if (!trigger_start) {
                if (next_byte == '\n' || persist_trigger) trigger_start = true;
            }
        }
...
private static void trigger(String event) {
         if (event.equals("Broadcasting is now enabled."))  broadcasting = true;
    else if (event.equals("Broadcasting is now disabled.")) broadcasting = false;
}
...
public static boolean step_server() {
    ...
    if (server == null) {
        if (question >= questions.length) {
            send(client, "Connecting to "+answers[MUD_HOST]+":"+answers[MUD_PORT]+".\n\r");
            if ( (server = connect_to(answers[MUD_HOST], answers[MUD_PORT])) == null) {
                send(client, "Failed to connect to "+answers[MUD_HOST]+":"+answers[MUD_PORT]+" (server).\n\r");                
                longsleep = true;
                try {
                    client.getOutputStream().flush();
                } catch (IOException e) {
                ...
                send(client, "\n\r");
                send(client, " o----------------------------------------------------------------------------o\n\r");
                send(client, " |   You are now multiplaying like a boss. To send commands to the MultiPlay  |\n\r");
                send(client, " |  server, use the dollar sign prefix. For example, $help will show you all  |\n\r");
                send(client, " | available commands and their short descriptions. Most likely you will only |\n\r");
                send(client, " |   ever want to use the $team, $chat, $subscribe and $broadcast commands.   |\n\r");
                send(client, " o----------------------------------------------------------------------------o\n\r");
                send(client, "\n\r");
            }
        }
    ...
    try {               
        ...
        if (next_byte == -1) log("MUD server disconnected us.");
    } catch (SocketTimeoutException e) {
        return false;
    } catch (IOException e) {
...
public static Socket wait_client(ServerSocket acceptor) {
    ...
    try {
        ...
        for (int i=0; i<questions.length; i++) answers[i] = "";
        answers[MUD_HOST] = "stonia.ttu.ee";
        answers[MUD_PORT] = "4000";
        answers[MPS_HOST] = "62.163.212.157";
        answers[MPS_PORT] = "4000";

        log("New connection #"+client_nr+" from "+client.getInetAddress().getHostAddress()+":"+client.getPort()+".");
...
public static Socket connect_to(String host, String port) {
    ...
    try {
        ...
        log("Connected to "+s.getInetAddress().getHostName()+":"+s.getPort()+".");
...
public static void close_client() {
    ...
    try {
        client.close();
        log("Disconnected client #"+client_nr+" ("+client.getInetAddress().getHostAddress()+":"+client.getPort()+").");
    }
    catch (IOException e) {
...
public static void close_server() {
    ...
    String place = server.getInetAddress().getHostName()+":"+server.getPort();
    try {
        server.close();
...
public static void close_multiplay() {
    if (multiplay == null) return;
    String place = multiplay.getInetAddress().getHostName()+":"+multiplay.getPort();
    try {
        multiplay.close();
...
public static void log(String text) {
    ...
    System.out.printf("%1$ta %1$tb %1$td %1$tH:%1$tM:%1$tS %1$tY :: %2$s\n", date, text);
}

public static void bug(String text) {
    String methodName = Thread.currentThread().getStackTrace()[2].getMethodName();
    int lineNumber    = Thread.currentThread().getStackTrace()[2].getLineNumber();
    text = methodName+" (line "+lineNumber+"): "+text;

    longsleep = true;
    Date date = Calendar.getInstance().getTime();
    System.err.printf("\u001B[1;31m%1$ta %1$tb %1$td %1$tH:%1$tM:%1$tS %1$tY ::\u001B[0m %2$s\n", date, text);
}
